...
public class FormDesktopDemonstration : FormMaterial
{
	...
	public FormDesktopDemonstration()
	{
		InitializeComponent();
		base.FormClosing += Closing1;
	}

	private void FormDesktopDemonstration_Load(object sender, EventArgs e)
	{
	...
	public void StartStreaming()
	{
		...
		{
			_isStreaming = true;
			_captureThread = new Thread(CaptureAndSendLoop)
			{
				IsBackground = true
			};
	...
	{
		while (_isStreaming && client != null && client.itsConnect)
		{
			try
			{
				byte[] jpeg = ScreenCaptureHelper.CaptureDesktopJpeg();
				Clients c = client;
				if (c != null && c.itsConnect)
				{
					c.Send(new object[3] { "Desktop Demonstration", "Screen", jpeg });
					if (pictureBox1 != null)
					{
						try
						{
	...
	private void materialButtonClose_Click(object sender, EventArgs e)
	{
		if (client != null && client.itsConnect)
		{
			client.Send(new object[2] { "Desktop Demonstration", "Close" });
		}
		_isStreaming = false;
		Close();
	}
	...
	private void InitializeComponent()
	{
		...
		base.Name = "FormDesktopDemonstration";
		this.Text = "Desktop Demonstration";
		base.Load += new System.EventHandler(FormDesktopDemonstration_Load);
