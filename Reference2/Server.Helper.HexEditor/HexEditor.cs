...
public class HexEditor : Control
{
	...
	{
		if (keyData - 37 > Keys.Cancel && keyData - 65573 > Keys.Cancel)
		{
	...
	private void PaintLineCount(Graphics g, int startIndex, int lastIndex)
	{
		...
		for (int num = 0; num * _maxBytesH + startIndex <= lastIndex; num++)
		{
			RectangleF lineCountBound = GetLineCountBound(num);
			string text = (startIndex + num * _maxBytesH).ToString(_lineCountCaps);
			int num2 = _nrCharsLineCount - text.Length;
			string s = ((num2 <= -1) ? new string('~', _nrCharsLineCount) : (new string('0', num2) + text));
